...
Every increase in income is associate with a -2.1% decrease in likelihood of voting democrat when controlling for other factors..
...
It's likely that once your income rises, you will be less likely to prefer a democratic candidate, but this depends on other factors as well, such as your race and gender.
...
If you are a democrat, you are 79% likely to prefer the democratic candidate. IF you are republican that number drops to 28% unless you are not white in which the number increases to 88%.
...
500, no because it levels off at around 30 trees. 500 might actually be unnecessarily large.

What is OOB error?
The model has an error rate of 25.45% 
...
nes_preds <- nes %>%
  mutate(logistic = predict(logistic_fit, new_data = nes) %>% 
           pull(.pred_class),
         tree = predict(house_region_tree, new_data = nes) %>% 
           pull(.pred_class),
         forest = predict(house_forest, new_data = nes) %>% 
           pull(.pred_class))

tibble(logistic = mean(nes_preds$logistic == nes_preds$vote),
       tree = mean(nes_preds$tree == nes_preds$vote),
       forest = mean(nes_preds$forest == nes_preds$vote))
```
Note:
